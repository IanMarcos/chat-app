    ...
    const updateUser = async ({ value: uid }) => {
      if (uid) {
        ...
        try {
          ...
          const [
            {
              results: { err, updatedUser },
            },
            {
              results: { cvToken },
            },
          ] = await Promise.all([updateResponse.json(), tokenResponse.json()]);
          ...
          basicNotification(
            `El usuario ${updatedUser.name} con correo ${updatedUser.email} ha sido Actualizado`
          );
      ...
      <form onSubmit={handleOnSubmit}>
        <div className="col">
          ...
          <div className="row justify-content-center my-2">
            <button className="btn btn-secondary w-50" onClick={handleExit}>
              Volver
            </button>
          </div>
        </div>
